...
// ── lower_protocol_fee_bps ──────────────────────────────────────────────────

#[test]
fn test_lower_protocol_fee_bps_success() {
    let env = Env::default();
    let (client, admin, sme) = setup(&env);
    let (token, treasury) = free_addresses(&env);
    ...
    client.init(
        ...
        &None,
        &None,
        &Some(1_000i64), // initial protocol fee: 10%
    );

    assert_eq!(client.get_protocol_fee_bps(), 1_000i64);

    let new_bps = client.lower_protocol_fee_bps(&200i64);
    assert_eq!(new_bps, 200i64);
    assert_eq!(client.get_protocol_fee_bps(), 200i64);
}

#[test]
fn test_lower_protocol_fee_bps_reduces_actual_withdraw_fee() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.init(
        ...
        &None,
        &None,
        &Some(1_000i64), // 10% initial fee
    );

    // Admin lowers fee from 1000 bps (10%) to 200 bps (2%) before funding/withdrawing
    client.lower_protocol_fee_bps(&200i64);
    assert_eq!(client.get_protocol_fee_bps(), 200i64);

    // Fund escrow to target
    let investor = Address::generate(&env);
    sac_admin.mint(&investor, &target);
    client.fund(&investor, &target);
    sac_admin.mint(&escrow_id, &target); // mint coupon headroom for withdraw

    // SME withdraws
    client.withdraw();

    // 2% of 100,000,000,000 is 2,000,000,000 (treasury cut)
    // 98% of 100,000,000,000 is 98,000,000,000 (sme payout)
    let treasury_balance = token_client.balance(&treasury);
    let sme_balance = token_client.balance(&sme);

    assert_eq!(treasury_balance, 2_000_000_000i128);
    assert_eq!(sme_balance, 98_000_000_000i128);
}

#[test]
fn test_lower_protocol_fee_bps_rejects_raise() {
    let env = Env::default();
    let (client, admin, sme) = setup(&env);
    let (token, treasury) = free_addresses(&env);
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_RAISE"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    assert_contract_error(
        client.try_lower_protocol_fee_bps(&800i64),
        EscrowError::ProtocolFeeBpsNotLower,
    );
}

#[test]
fn test_lower_protocol_fee_bps_rejects_same_bps() {
    let env = Env::default();
    let (client, admin, sme) = setup(&env);
    let (token, treasury) = free_addresses(&env);
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_SAME"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    assert_contract_error(
        client.try_lower_protocol_fee_bps(&500i64),
        EscrowError::ProtocolFeeBpsNotLower,
    );
}

#[test]
fn test_lower_protocol_fee_bps_rejects_out_of_range_negative() {
    let env = Env::default();
    let (client, admin, sme) = setup(&env);
    let (token, treasury) = free_addresses(&env);
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_NEG"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    assert_contract_error(
        client.try_lower_protocol_fee_bps(&-1i64),
        EscrowError::ProtocolFeeBpsOutOfRange,
    );
}

#[test]
fn test_lower_protocol_fee_bps_rejects_out_of_range_above_10000() {
    let env = Env::default();
    let (client, admin, sme) = setup(&env);
    let (token, treasury) = free_addresses(&env);
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_OVER"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    assert_contract_error(
        client.try_lower_protocol_fee_bps(&10_001i64),
        EscrowError::ProtocolFeeBpsOutOfRange,
    );
}

#[test]
fn test_lower_protocol_fee_bps_accepts_zero() {
    let env = Env::default();
    let (client, admin, sme) = setup(&env);
    let (token, treasury) = free_addresses(&env);
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_ZERO"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    let new_bps = client.lower_protocol_fee_bps(&0i64);
    assert_eq!(new_bps, 0i64);
    assert_eq!(client.get_protocol_fee_bps(), 0i64);
}

#[test]
fn test_lower_protocol_fee_bps_rejects_when_withdrawn() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_WD_ERR"),
        &sme,
        &target,
        &800i64,
        &0u64,
    ...
    sac_admin.mint(&investor, &target);
    client.fund(&investor, &target);
    sac_admin.mint(&client.address, &target);

    // SME withdraws, setting status to 3
    client.withdraw();
    assert_eq!(client.get_escrow().status, 3);

    // Attempting to lower protocol fee after withdrawal must panic with ProtocolFeeLowerWithdrawn
    assert_contract_error(
        client.try_lower_protocol_fee_bps(&100i64),
        EscrowError::ProtocolFeeLowerWithdrawn,
    );
}

#[test]
fn test_lower_protocol_fee_bps_allowed_in_open_funded_settled_cancelled() {
    let env = Env::default();
    env.mock_all_auths();

    // 1. Open state (status == 0)
    let (client, admin, sme) = setup(&env);
    let (token, treasury) = free_addresses(&env);
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_ST0"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    assert_eq!(client.lower_protocol_fee_bps(&700i64), 700i64);

    // 2. Funded state (status == 1)
    let (client_f, admin_f, sme_f) = setup(&env);
    let sac_f = env.register_stellar_asset_contract_v2(Address::generate(&env));
    let token_id_f = sac_f.address();
    let sac_admin_f = StellarAssetClient::new(&env, &token_id_f);
    let treasury_f = Address::generate(&env);
    let target_f = 100_000_000_000i128;
    client_f.init(
        &admin_f,
        &String::from_str(&env, "FEE_LO_ST1"),
        &sme_f,
        &target_f,
        &800i64,
        &0u64,
    ...
    let investor_f = Address::generate(&env);
    sac_admin_f.mint(&investor_f, &target_f);
    client_f.fund(&investor_f, &target_f);
    assert_eq!(client_f.get_escrow().status, 1);
    assert_eq!(client_f.lower_protocol_fee_bps(&600i64), 600i64);

    // 3. Settled state (status == 2)
    let (client_s, admin_s, sme_s) = setup(&env);
    let sac_s = env.register_stellar_asset_contract_v2(Address::generate(&env));
    let token_id_s = sac_s.address();
    let sac_admin_s = StellarAssetClient::new(&env, &token_id_s);
    let treasury_s = Address::generate(&env);
    let target_s = 100_000_000_000i128;
    client_s.init(
        &admin_s,
        &String::from_str(&env, "FEE_LO_ST2"),
        &sme_s,
        &target_s,
        &800i64,
        &0u64,
    ...
    let investor_s = Address::generate(&env);
    sac_admin_s.mint(&investor_s, &target_s);
    client_s.fund(&investor_s, &target_s);
    client_s.settle();
    assert_eq!(client_s.get_escrow().status, 2);
    assert_eq!(client_s.lower_protocol_fee_bps(&500i64), 500i64);

    // 4. Cancelled state (status == 4)
    let (client_c, admin_c, sme_c) = setup(&env);
    let (token_c, treasury_c) = free_addresses(&env);
    client_c.init(
        &admin_c,
        &String::from_str(&env, "FEE_LO_ST4"),
        &sme_c,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    client_c.cancel_funding();
    assert_eq!(client_c.get_escrow().status, 4);
    assert_eq!(client_c.lower_protocol_fee_bps(&400i64), 400i64);
}

#[test]
fn test_lower_protocol_fee_bps_requires_admin_auth() {
    let env = Env::default();
    let (client, admin, sme) = setup(&env);
    let (token, treasury) = free_addresses(&env);
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_AUTH"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    assert!(
        env.auths().iter().any(|(addr, _)| *addr == admin),
        "admin auth was not recorded for lower_protocol_fee_bps"
    );
}
...
fn test_lower_protocol_fee_bps_unauthorized_typed_error() {
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_UNAUTH"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
...
#[test]
fn test_lower_protocol_fee_bps_emits_event() {
    use soroban_sdk::testutils::Events as _;
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_EVT"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    assert!(
        all_events
            .events()
            .contains(&expected.to_xdr(&env, &client.address)),
        "ProtocolFeeBpsLowered event must be emitted"
    );
}

#[test]
fn test_lower_protocol_fee_bps_successive_reductions() {
    let env = Env::default();
    let (client, admin, sme) = setup(&env);
    let (token, treasury) = free_addresses(&env);
    ...
    client.init(
        &admin,
        &String::from_str(&env, "FEE_LO_MULTI"),
        &sme,
        &100_000_000_000i128,
        &800i64,
        &0u64,
    ...
    assert_eq!(client.get_protocol_fee_bps(), 1_000i64);
    assert_eq!(client.lower_protocol_fee_bps(&800i64), 800i64);
    assert_eq!(client.get_protocol_fee_bps(), 800i64);
    assert_eq!(client.lower_protocol_fee_bps(&500i64), 500i64);
